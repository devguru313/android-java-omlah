    ...
    private static String[] monthsList() {
        if (MONTHS_LIST == null) {
            ...
            for (int i = 0; i < months.length; i++) {
                ...
                double add = 1;
                double current = Double.parseDouble(String.valueOf(i));
                double now = add + current;
                String format = String.valueOf(now).replace(".0","");
                stringMonths[i] = capitalize(monthName)+" [ "+format+" ]";
            }
